            ...
            if (testFilePath.includes("*")) {
                ...
                glob(testFilePath, options, function (er, currentFilePaths) {
                    for (const path of currentFilePaths) {
                        ...
                        fs.readdir(`./${TESTS_FOLDER}/99x-Inventory-master-template-javascript/tests`, (err, originalFiles) => {
...
const copyFile = (srcFileName, destPath) => {
    fs.copyFile(`${TESTS_FOLDER}/99x-Inventory-master-template-javascript/tests/${srcFileName}`, `../${destPath}`, (err) => {
        if (err) throw err;
    });
    return "Copied";
}
